...
def test_calendar_capacity_counts_live_phone_slots_and_excludes_aurelie(
    tmp_path, monkeypatch
):
    client = authenticated_client(tmp_path, monkeypatch)
    data = application.load_data()
    data["crm_calendly"] = {
        "user": "https://api.calendly.com/users/CLEMENT",
        "organization": "https://api.calendly.com/organizations/ORG1",
        "scope": "organization",
    }
    data["crm_calendly_appointments"] = [
        {
            "id": "phone-cassandre",
            "status": "active",
            "name": "RDV téléphonique formation APS",
            "start_time": "2099-08-12T08:00:00Z",
            "end_time": "2099-08-12T08:30:00Z",
            "host_name": "Cassandre Menard",
            "host_email": "cassandre@integraleacademy.com",
        ...
        },
        {
            ...
            "status": "active",
            "name": "RDV téléphonique formation APS",
            "start_time": "2099-08-12T09:00:00Z",
            "host_name": "Aurélie CHAUSSEZ",
            "host_email": "aurelie@integraleacademy.com",
            "location": {"type": "outbound_call"},
        },
        {
            "id": "zoom-cassandre",
            "status": "active",
            "name": "Entretien Zoom",
            "start_time": "2099-08-12T10:00:00Z",
            "host_name": "Cassandre Menard",
            "location": {"type": "zoom"},
        },
        {
            ...
            "status": "canceled",
            "name": "Appel découverte",
            "start_time": "2099-08-12T11:00:00Z",
            "host_name": "Cassandre Menard",
            "location": {"type": "outbound_call"},
        },
        {
            ...
            "status": "active",
            "name": "Appel découverte",
            "start_time": "2099-08-13T08:00:00Z",
            "host_name": "Cassandre Menard",
            "location": {"type": "outbound_call"},
        },
    ]
    ...
    def fake_calendly(method, path, **kwargs):
        if path == "/organization_memberships":
            assert kwargs["params"]["email"] == "aurelie@integraleacademy.com"
            return {
                "collection": [{
                    "user": {
                        "uri": "https://api.calendly.com/users/AURELIE",
        ...
        if path == "/event_types":
            return {
                "collection": [
                    {
                        "uri": "https://api.calendly.com/event_types/APS",
                        "name": "RDV téléphonique formation APS",
                        "active": True,
                        "locations": [{"kind": "outbound_call"}],
                        "profile": {
                            "name": "Cassandre Menard",
                            "owner": "https://api.calendly.com/users/CASSANDRE",
                        },
                    },
                    {
                        "uri": "https://api.calendly.com/event_types/A3P",
                        "name": "RDV téléphonique formation A3P",
                        "active": True,
                        "locations": [{"kind": "custom", "location": "Appel téléphonique"}],
                        "profile": {
                            "name": "Clément Vaillant",
                            "owner": "https://api.calendly.com/users/CLEMENT",
                        },
                    },
                    {
                        "uri": "https://api.calendly.com/event_types/AURELIE",
                        "name": "RDV téléphonique BTS",
                        "active": True,
                        "locations": [{"kind": "outbound_call"}],
                        "profile": {
                            "name": "Équipe BTS",
                            "owner": "https://api.calendly.com/users/AURELIE",
                        },
                    },
                    {
                        "uri": "https://api.calendly.com/event_types/ZOOM",
                        "name": "Entretien Zoom",
                        "active": True,
                        "locations": [{"kind": "zoom_conference"}],
                        "profile": {"name": "Cassandre Menard"},
                    },
                ],
                "pagination": {"next_page_token": None},
            }
        if path == "/event_type_available_times":
            event_type = kwargs["params"]["event_type"]
            availability_calls.append(event_type)
            slots = {
                "https://api.calendly.com/event_types/APS": [
                    "2099-08-12T12:00:00Z",
                    "2099-08-12T13:00:00Z",
                ],
                "https://api.calendly.com/event_types/A3P": [
                    "2099-08-12T12:00:00Z",
                    "2099-08-12T14:00:00Z",
                ],
            }[event_type]
            return {"collection": [
                {"status": "available", "start_time": value}
                for value in slots
            ]}
        raise AssertionError(f"Unexpected Calendly call: {method} {path}")
    ...
    monkeypatch.setattr(application, "_calendly_request", fake_calendly)
    response = client.get(
        "/api/crm/calendly/calendar-capacity?start_date=2099-08-12"
    )

    assert response.status_code == 200
    payload = response.get_json()
    assert payload["live"] is True
    assert payload["excluded_host"] == "Aurélie Chaussez"
    assert payload["event_type_count"] == 2
    assert [item["id"] for item in payload["appointments"]] == [
        "phone-cassandre", "phone-next-day"
    ]
    ...
    assert payload["days"][1]["appointment_count"] == 1
    assert set(availability_calls) == {
        "https://api.calendly.com/event_types/APS",
        "https://api.calendly.com/event_types/A3P",
    }


def test_calendar_capacity_keeps_appointments_when_calendly_is_unavailable(
    tmp_path, monkeypatch
):
    client = authenticated_client(tmp_path, monkeypatch)
    data = application.load_data()
    ...
    monkeypatch.setattr(
        application,
        "_crm_calendly_calendar_excluded_user_uris",
        lambda stored: set(),
    )
    monkeypatch.setattr(
        application,
        "_calendly_event_types_for_context",
        lambda stored: (_ for _ in ()).throw(
            application.CalendlyAPIError(
                503, {"title": "Unavailable", "message": "Calendly indisponible"}
            )
        ),
    )
    ...
    assert payload["days"][0]["appointment_count"] == 1
    assert payload["days"][0]["available_count"] is None
    assert payload["appointments"][0]["id"] == "cached-phone"
    assert "Calendly indisponible" in payload["warning"]


def test_calendar_capacity_rejects_an_invalid_start_date(tmp_path, monkeypatch):
    response = authenticated_client(tmp_path, monkeypatch).get(
        "/api/crm/calendly/calendar-capacity?start_date=31-08-2026"
    )

    assert response.status_code == 400
    assert "date de début" in response.get_json()["error"]


def test_calendar_page_renders_live_daily_capacity():
    ...
    for marker in (
        "function calendarCapacityDayCard",
        "function calendarCapacityOverview",
        "/api/crm/calendly/calendar-capacity?start_date=",
        "Rendez-vous du jour",
        "Encore disponibles",
        "Capacité totale du jour",
        "Aurélie Chaussez exclue des calculs",
        "calendarRequestSequence",
    ):
        assert marker in crm_js
    for marker in (
        ".calendar-capacity{",
        ".calendar-capacity-days{",
        ".calendar-capacity-metrics{",
        ".calendar-capacity-live.is-live",
    ):
        assert marker in crm_css
    assert crm_template.count(
        "calendar_capacity_version='20260830-calendar-live-capacity-1'"
    ) == 2

    helpers = crm_js[
        crm_js.index("function calendarCapacityDayCard"):
        crm_js.index("function changeCalendarDate")
    ]
    script = r"""
const assert=require('node:assert/strict');
const calendarSelectedDate='2099-08-12';
const esc=value=>String(value??'');
""" + helpers + r"""
const rendered=calendarCapacityOverview({
  live:true,
  refreshed_at:'2099-08-12T10:15:00+02:00',
  days:[{
    date:'2099-08-12', appointment_count:1, available_count:3,
    total_capacity:4, availability_status:'live'
  }]
});
assert.match(rendered,/3 créneaux disponibles/);
assert.match(rendered,/1 RDV prévu/);
assert.match(rendered,/Capacité totale du jour/);
assert.match(rendered,/Aurélie Chaussez exclue des calculs/);
"""
    completed = subprocess.run(
        ["node", "-e", script], check=False, capture_output=True, text=True,
    )
