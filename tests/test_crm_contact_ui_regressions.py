...
def test_legacy_cpf_badge_is_removed_below_converted_stage():
    javascript = CRM_JS.read_text(encoding="utf-8")
    stylesheet = CRM_CSS.read_text(encoding="utf-8")
    helper = javascript[
        javascript.index("function updateFundingBadges(c,resources=[]){"):
        javascript.index("function refreshFundingBadges(c)")
    ]
    script = r"""
const assert=require('node:assert/strict');
const root={innerHTML:'stale',hidden:false};
const document={querySelector:()=>root};
const wedofFundingSummary=resources=>resources.length?{latest:resources[0],cpf:'validated',ft:''}:{latest:null,cpf:'',ft:''};
""" + helper + r"""
updateFundingBadges({cpf:'OUI',financement_ft:'NON'},[{payload:{state:'validated'}}]);
assert.equal(root.innerHTML,'');
assert.equal(root.hidden,true);
updateFundingBadges({financement_ft:'OUI',statut_demande_financement_ft:'en_cours_instruction'},[]);
assert.match(root.innerHTML,/Demande FT en cours d’instruction/);
...
"""
    ...
    assert completed.returncode == 0, completed.stderr
    assert 'id="fundingBadges" aria-live="polite" hidden' in javascript
    assert "CPF validé · en attente du candidat" not in helper
    assert "CPF accepté" not in helper
    assert "Dossier CPF validé" not in helper
    assert "CPF renseigné" not in helper
    assert ".funding-badge.cpf" not in stylesheet
    assert ".funding-badges[hidden]{display:none}" in stylesheet
...
def test_activity_log_centralizes_every_contact_event_and_delegates_more():
    ...
    assert "visibleActivityKinds" not in javascript
    assert "Publication ajoutée" in javascript
    assert "Commentaire sur une publication" in javascript
    assert "Aucune activité enregistrée" in javascript
...
def test_contact_supports_a_removable_secondary_timeline():
    ...
    assert "'C2P en cours','Transition pro','Marché FT'" in javascript
...
def test_registration_appointment_moves_to_secondary_timeline_and_migrates_history():
    ...
    assert namespace["CRM_SECONDARY_STATUSES"][-2:] == ("Transition pro", "Marché FT")
    configured = {
        "crm_statuses": [
            "Nouveaux",
            "Prochain RDV inscription",
...
def test_contact_header_displays_all_activity_counters_including_zero():
    ...
    assert "['RDV',counts.appointments]" in crm_js
    assert "RDV total" not in crm_js
    assert "RDV réalisés" not in crm_js
...
def test_contact_header_offers_an_orange_five_minute_sms_action():
    crm_js = CRM_JS.read_text(encoding="utf-8")
    crm_css = CRM_CSS.read_text(encoding="utf-8")

    sms_position = crm_js.index('id="smsBtn"')
    quick_position = crm_js.index('id="quickReminderBtn"')
    calendar_position = crm_js.index('id="calendarBtn"')
    assert sms_position < quick_position < calendar_position
    assert 'aria-label="Envoyer le SMS Rappel dans 5min"' in crm_js
    assert "/quick-reminder" in crm_js
    assert "quickReminderButton.disabled=true" in crm_js
    assert "quickReminderButton.setAttribute('aria-busy','true')" in crm_js
    assert "SMS « Rappel dans 5min » envoyé" in crm_js
    assert ".contact-quick-actions>.quick-reminder-action" in crm_css
    assert "background:#f97316" in crm_css
...
def test_contact_completeness_modal_lists_every_missing_requirement():
    ...
    assert "require('next_action'" not in workspace
    assert "{title:'Suivi',keys:['origine']}" in workspace
    assert "item.key==='next_action'" not in workspace
...
"""
    ...
    assert "20260903-remove-duplicate-cpf-badge-1" in application
...
const activeWithoutFollowup={...base,statut:'En cours'};
assert(contactCompleteness(activeWithoutFollowup)===100,'a follow-up is not part of dossier completeness');
assert(contactCompleteness({...activeWithoutFollowup,relance_date:'2026-09-10'})===100,'planning a follow-up does not change dossier completeness');
...
"""
    ...
    assert 'CRM_ASSET_VERSION = "20260903-remove-duplicate-cpf-badge-1"' in application
...
"""
    ...
    assert "if(b.dataset.primaryStep==='Converti')return convertContact(c)" in javascript
