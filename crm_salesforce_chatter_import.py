...
def _plain_text(value: Any) -> str:
    ...
    cleaned_lines = [re.sub(r"[ \t]+", " ", line).strip() for line in rendered.split("\n")]
    result: list[str] = []
    blank = False
...
def _prepare_publications(
    ...
    for feed_item_id, comments in comments_by_feed_item.items():
        if feed_item_id in prepared_ids or not comments:
            continue
        ...
        prepared.append({
            ...
            "date": _text(comments[0].get("date")),
            ...
            "salesforce_comment_count": len(comments),
        })
    ...
    result["publication_type_counts"] = dict(type_counts.most_common())
    ...
    *,
    ...
    result = {
        ...
        "ready_rows": sorted(
            ready_rows.values(),
            key=lambda item: (
                -int(item.get("publications") or 0),
                item.get("person") or "",
            ),
        ),
            ...
            if dry_run:
                ...
                with shared_lock:
                    ...
                    if not supplied_token:
                        return jsonify({
                            "error": (
                                "Un aperçu doit être validé avant l’import de l’historique."
                            )
                        }), 409
