...
function showContact(id){
  ...
  <section class="card dossier-summary"><div class="card-head"><div><h2>✦ Synthèse du dossier</h2><small>Résumé généré par l’IA à partir des informations du dossier</small></div><button class="btn" id="summaryBtn">Générer la synthèse</button></div><div id="summaryText" class="summary-text">Cliquez sur « Générer la synthèse » pour obtenir un aperçu du dossier.</div></section>
  ...
  <div class="form-section"><h3>Suivi</h3><div class="section-fields"><div class="field"><label>Origine</label><select name="origine"><option></option>${['Google','Réseaux sociaux','CPF','FT','POEI','Autre','Calendly'].map(x=>`<option ${c.origine===x?'selected':''}>${x}</option>`).join('')}</select></div><div class="field full"><div class="field-label-row"><label>Commentaires</label><button type="button" class="ai-link" id="rephraseComments">✦ Reformuler avec l’IA</button></div><textarea name="commentaires" placeholder="Notes, besoins, contexte…">${esc(c.commentaires)}</textarea></div></div></div></form></section><aside class="card"><div class="card-head"><h2>Journal d’activité</h2></div><div class="feed" id="activityFeed">${feed(c)}</div></aside></div>`;
  bindContact(c);
}
function selectHtml(n,l,c){return `<label>${l}</label><div class="binary-choice" role="radiogroup" aria-label="${esc(l)}"><label><input type="radio" name="${n}" value="OUI" ${c[n]==='OUI'?'checked':''}>OUI</label><label><input type="radio" name="${n}" value="NON" ${c[n]==='NON'?'checked':''}>NON</label></div>`}function selectField(n,l,c){return `<div class="field">${selectHtml(n,l,c)}</div>`}function feed(c,expanded=false){const activities=c.activities||[],visible=expanded?activities:activities.slice(0,5);return visible.map(a=>`<div class="feed-item"><span class="feed-icon">${a.kind==='appel'?'☎':a.kind==='email'?'✉':a.kind==='sms'?'▣':a.kind==='calendly'?'◷':'◆'}</span><div><b>${esc(a.title)}</b><p>${esc(a.detail)}</p><time>${fmt(a.date)} · ${esc(a.author)}</time>${a.preview?`<br><a class="preview-link" data-preview="${encodeURIComponent(a.preview)}">Prévisualiser</a>`:''}</div></div>`).join('')+(activities.length>5&&!expanded?`<button class="btn feed-more" id="feedMore">Voir plus (${activities.length-5})</button>`:'')||'<div class="empty">Aucune activité</div>'}
...
function bindContact(c){
  summaryBtn.onclick=async()=>{summaryBtn.disabled=true;summaryBtn.textContent='Génération…';try{summaryText.textContent=(await api(`/api/crm/contacts/${c.id}/synthese`,{method:'POST',body:'{}'})).texte}catch(e){toast(e.message,true)}finally{summaryBtn.disabled=false;summaryBtn.textContent='Actualiser la synthèse'}};
  ...
  const more=document.querySelector('#feedMore');if(more)more.onclick=()=>{activityFeed.innerHTML=feed(c,true);document.querySelectorAll('[data-preview]').forEach(a=>a.onclick=()=>previewModal(decodeURIComponent(a.dataset.preview)))};
...
function renderCalendlyAppointments(c,items,integration){
  ...
  const sections=`<details class="appointment-history"><summary>Voir tous les rendez-vous <span>${Math.max(0,ordered.length-(next?1:0))}</span></summary><div class="appointment-sections">${calendlyAppointmentSection('Autres rendez-vous à venir',upcoming.slice(1),'upcoming')}${calendlyAppointmentSection('Rendez-vous passés',past,'past')}${calendlyAppointmentSection('Rendez-vous annulés',canceled,'canceled')}</div></details>`;
